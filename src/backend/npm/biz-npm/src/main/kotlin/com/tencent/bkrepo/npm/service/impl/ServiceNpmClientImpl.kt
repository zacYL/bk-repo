...
class ServiceNpmClientImpl : ServiceNpmClientService, AbstractNpmService() {
    ...
    override fun deleteVersion(
        ...
        val tgzPath = packageService.findVersionByName(projectId, repoName, packageKey, version)?.contentPath
            ?: return
        // 删除包管理中对应的version
        packageService.deleteVersion(projectId, repoName, packageKey, version, null)
        // 删除文件节点
        val repoType = ArtifactContextHolder.getRepoDetail(RepositoryId(projectId, repoName)).type
        val name = NpmUtils.resolveNameByRepoType(packageKey, repoType)
        val metadataPath = NpmUtils.getVersionPackageMetadataPath(name, version)
        nodeService.deleteNode(NodeDeleteRequest(projectId, repoName, tgzPath, operator))
        nodeService.deleteNode(NodeDeleteRequest(projectId, repoName, metadataPath, operator))
    ...
    private fun updatePackageJson(
        ...
        val repositoryDetail = ArtifactContextHolder.getRepoDetail(RepositoryId(projectId, repoName))
            ?: throw RepoNotFoundException("repo not found: { projectId=$projectId, repoName=$repoName }")
        val name = NpmUtils.resolveNameByRepoType(packageKey, repositoryDetail.type)
        val packageFullPath = NpmUtils.getPackageMetadataPath(name)
        val node = nodeService.getNodeDetail(ArtifactInfo(projectId, repoName, packageFullPath)) ?: return
        ...
        storageManager.loadArtifactInputStream(node, repositoryDetail.storageCredentials).use {
            ...
            if (version != latest) {
                ...
                val newLatest = packageService.findPackageByKey(projectId, repoName, packageKey)?.latest
                newLatest?.let {
