        ...
        <draggable v-model="proxyList" :options="{ animation: 200 }">
            <div class="proxy-item" v-for="proxy in proxyList" :key="proxy.name + Math.random()">
                <div class="proxy-index flex-align-center">
                    <i class="ml10 devops-icon icon-more"></i>
                    <i class="devops-icon icon-more" style="margin-left:-10px"></i>
                </div>
...
<script>
    ...
    export default {
        ...
        watch: {
            baseData (val) {
                this.proxyList = JSON.parse(JSON.stringify(this.baseData.configuration.proxy.channelList))
            }
        },
        created () {
            this.proxyList = JSON.parse(JSON.stringify(this.baseData.configuration.proxy.channelList))
