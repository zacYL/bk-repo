...
export function customizeExportScanFile (url, methodType, language, successNotify, errorNotify) {
    ...
    axios({
        ...
        if (res.headers['content-disposition']) {
            ...
            vm.$bkNotify({
                title: successNotify || vm.$t('exportFileLoadingInfo'),
        ...
        if (e.response.data.type === 'application/json') {
            ...
            // 后端返回的数据类型不是 json
            vm.$bkMessage({
                message: errorNotify || vm.$t('exportFileErrorInfo'),
                theme: 'error'
            })
        }
    })
