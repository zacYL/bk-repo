...
@Api("平台权限自定义角色接口")
@Primary
@FeignClient(DEVOPS_AUTH_SERVICE_NAME, contextId = "CanwayCustomPermissionClient")
...
interface CanwayCustomPermissionClient {
    ...
    fun saveAnyPermissions(
        ...
        subjectId: String,
        @PathVariable
        @ApiParam(value = "资源类型（project、tenant、system）", required = true)
    ...
    fun removeResourcePermissions(
        @RequestHeader(AUTH_HEADER_DEVOPS_UID)
        @ApiParam(value = "删除者", required = true)
        userId: String,
        @RequestBody
