...
abstract class ArtifactExtService : ArtifactService() {
    ...
    @Autowired
    lateinit var packageAccessRuleService: PackageAccessRuleService
    ...
    open fun moveCopyVersion(request: PackageVersionMoveCopyRequest, move: Boolean) {
        with(request) {
            ...
            val srcPackage = packageService.findPackageByKey(srcProjectId, srcRepoName, packageKey)!!
    ...
    protected fun getAndCheckRepository(
        ...
        val srcRepo = repositoryService.getRepoDetail(srcProjectId, srcRepoName)
            ?: throw RepoNotFoundException("$srcProjectId/$srcRepoName")
        val dstRepo = repositoryService.getRepoDetail(dstProjectId, dstRepoName)
    ...
    protected open fun getAndCheckSrcVersion(
        ...
        val srcVersion = packageService.findVersionByName(projectId, repoName, packageKey, version)
    ...
    protected fun checkDst(
        ...
        packageService.findVersionByName(dstRepo.projectId, dstRepo.name, packageKey, version)?.let { versionInfo ->
    ...
    private fun checkSrcVersion(
        ...
        if (
            ...
            packageAccessRuleService.checkPackageAccessRule(projectId, packageKey, version) != true
